...
def print_tree(root, depth=0):
    print('  ' * depth + root['info']['tax_name'])
...
@click.command()
@click.option('-k', '--min-kmer', default=0)
@click.option('-a', '--min-abundance', default=0.0)
@click.option('-c', '--min-cov', default=0.0)
@click.option('-w/-l', '--wide/--long', default=True)
@click.option('-r', '--rank', default=None, help='Select only a specific taxa rank.')
@click.option('-f', '--features', default='reads,kmers,dup,cov,percent,tax_reads',
              help='Comma separated list of features reads,kmers,dup,cov,percent,tax_reads')
@click.argument('out_file', type=click.File('w'))
@click.argument('report_filenames', nargs=-1)
def main(min_abundance, min_kmer, min_cov, wide, rank, features, out_file, report_filenames):
    try:
        report_files = {
            get_sample_name(report_filename): open(report_filename)
        ...
        out_file.write(tbl.to_csv())
        click.echo(f'Wrote table with shape {tbl.shape}', err=True)
    except Exception:
